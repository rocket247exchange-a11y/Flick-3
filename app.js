...
/* === Movie data (Now Showing) ===
   NOTE: I used the GIFs/links you provided where possible.
   Replace poster or video URLs with direct files if you have them.
*/
const MOVIES = [
  {
    id: "madam-cash",
...
/* Coming soon (GIFs as backgrounds) */
const COMING = [
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // show welcome for ~4.5s then fade
  setTimeout(()=>{
    welcome.classList.add('welcome-hidden');
    // after fade out, hide element and reveal main
    setTimeout(()=>{
      welcome.setAttribute('aria-hidden','true');
      main.classList.remove('main-hidden');
      main.classList.add('main-visible');
      main.setAttribute('aria-hidden','false');
    }, 900); // must match welcome-hidden transition duration
  }, 4500);
});
